...
if ( ! class_exists( 'WR_GssWeeklyTraining' ) ) :

/**
 * Create GssWeeklyTraining element
 *
 * @package  GssWeeklyTraining Shortcodes
 * @since    1.0.0
 */
class WR_GssWeeklyTraining extends WR_Pb_Shortcode_Element {
  /**
   * Constructor
   *
   * @return  void
  ...
  function element_config() {
    ...
    $this->config['name']        = __( 'Weekly Training', WR_PBL );
    $this->config['icon']        = 'wr-icon-text';
    $this->config['description'] = __( 'Weekly Training Addon', WR_PBL );
  ...
  function element_shortcode_full( $atts = null, $content = null ) {
    ...
    $content .= WR_Pb_Helper_Shortcode::remove_autop( get_option( 'gss_training_times', '' ) );
