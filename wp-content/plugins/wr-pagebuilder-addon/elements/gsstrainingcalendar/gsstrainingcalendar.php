...
if ( ! class_exists( 'WR_GssTrainingCalendar' ) ) :

/**
 * Create GssTrainingCalendar element
 *
 * @package  GssTrainingCalendar Shortcodes
 * @since    1.0.0
 */
class WR_GssTrainingCalendar extends WR_Pb_Shortcode_Element {
  /**
   * Constructor
   *
   * @return  void
  ...
  function element_config() {
    ...
    $this->config['name']        = __( 'Training Calendar', WR_PBL );
    $this->config['icon']        = 'wr-icon-text';
    $this->config['description'] = __( 'Training Calendar Addon', WR_PBL );
  ...
  function element_shortcode_full( $atts = null, $content = null ) {
    ...
    $content .= WR_Pb_Helper_Shortcode::remove_autop( get_option( 'gss_training_calendar', '' ) );

    $html_element .= $content;
    $html_element = str_replace('<table>', '<table class="table table-hover" id="%s">', $html_element);
    $html  = sprintf( $html_element, $random_id );
    // $html .= $html_element;
    // $html .= '</table>';
