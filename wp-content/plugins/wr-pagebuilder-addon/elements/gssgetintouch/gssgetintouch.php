...
if ( ! class_exists( 'WR_GssGetInTouch' ) ) :
 ...
 * @since    1.0.0
 */
class WR_GssGetInTouch extends WR_Pb_Shortcode_Element {
	/**
	 * Constructor
	 *
	 * @return  void
	...
	function element_config() {
		...
		$this->config['name']        = __( 'Get in touch', WR_PBL );
		...
		$this->config['exception'] = array(
			'default_content' => __( 'Get in touch', WR_PBL ),
	...
	function element_items() {
		$this->items = array(
			'content' => array(
				...
				array(
					'name' => __( 'Element Text', WR_PBL ),
					'desc' => __( 'Enter some content for this textblock', WR_PBL ),
					'id'   => 'text',
					'type' => 'text_area',
					'role' => 'content',
					'std'  => get_option( 'gss_getintouch_text', 'GET IN TOUCH' ),
					'rows' => 15,
				),
			),
			'styling' => array(
	...
	function element_shortcode_full( $atts = null, $content = null ) {
		...
		if ( ! empty( $content ) ) {
			...
			$content = get_option( 'gss_getintouch_text', 'GET IN TOUCH' );
		}
		$content = sprintf('<p style="text-align: center;"><span>%s</span><span>%s</span> or <span>%s</span></p>',
			$content,
			get_option( 'gss_getintouch_phone', '+020 7148 1602' ),
			get_option( 'gss_getintouch_email', 'isleworth@wemakefootballers.com' )
		);
		// if ( ! empty( $content ) ) {
		// 	$content = WR_Pb_Helper_Shortcode::remove_autop( $content );
		// }
		...
		$arr_params['css_suffix'] .= ' wr-element-text wmf-info ';
