...
if ( ! class_exists( 'WR_Column' ) ) {
	...
	class WR_Column extends WR_Pb_Shortcode_Layout {
		...
		public function shortcode_data() {
			...
			$this->config['params']['span'] = ( ! empty( $this->params['span'] ) ) ? $this->params['span'] : 'span'.GSS_COLUMNS;
		...
		public function element_in_pgbldr( $content = '', $shortcode_data = '' ) {
			...
			$span           = ( ! empty( $this->params['span'] ) ) ? $this->params['span'] : 'span'.GSS_COLUMNS;
		...
		function element_shortcode( $atts = null, $content = null ) {
			...
			$class   = sprintf("wmf-col col-md-%s col-sm-%s col-xs-%s %s", $span, $span, GSS_COLUMNS, $hidden_on);
