...
class WR_Row extends WR_Pb_Shortcode_Layout {
	...
	function element_items() {
		...
		array(
					...
					'type'    => 'radio',
					'std'     => 'boxed',
					'options' => array( 'boxed' => __( 'Boxed', WR_PBL ), 'full' => __( 'Full', WR_PBL ), 'banner' => __( 'Banner', WR_PBL ) ),
		),
		array(
	...
	function element_shortcode( $atts = null, $content = null ) {
		...
		if ( isset( $atts ) && is_array( $atts ) ) {
			...
			switch ( $atts['width'] ) {
				case 'full':
					...
					$arr_styles[] = '-webkit-box-sizing: content-box;-moz-box-sizing: content-box;box-sizing: content-box;';
					break;
				case 'banner':
					$extra_class = 'wr_fullwidth localtion-banner-page local-banner-carousel';
					// some overwrite css to enable row full width
					// $script = "$('body').addClass('wr-full-width');";
					// $custom_script = WR_Pb_Helper_Functions::script_box( $script );
			...
			// if ( $atts['width'] == 'full' || $atts['width'] == 'banner' )
			// {
			// 	$style = str_replace(array('width:100%;','padding-left:10px;','padding-right:10px;','margin:0 -10px;'), '', $style);
			// }
		}
		...
		return $custom_script . sprintf(
				"<div class='jsn-bootstrap3 %s'><div id='%s' class='row %s' %s>%s</div></div>",
				( (isset($atts['width']) AND ( $atts['width']=='full' OR $atts['width']=='banner' )) ?  'container-fluid'  : 'container' ),
				$extra_id,
				ltrim( $extra_class, ' ' ),
				$data_attr . ' ' . $style,
