...
(function ($) {
	...
	JSNLayoutCustomizer.prototype = {
		...
		calStepWidth:function(countColumn, reset){
			...
			this.step = parseInt((this.maxWidth - (this.spacing * (formRowLength -1))) / this.gridGolumns);
		},
			...
			var grid_modal = new $.IGModal({
				...
				loaded: function (obj, iframe) {
					if ( $('#wr-add-element-modal').length == 0 ) {
						grid_modal.container.attr('id', 'wr-add-element-modal');
					}
					...
					var html_box = '<div class="col-xs-12"><div class="form-group"><label for="formGrid">Gird columns</label><input type="text" id="formGridColumns" class="form-control" placeholder="4,4,4,4"></div></div>';
					var html_box = '<div id="wr-shortcodes" class="wr-custom-grid jsn-bootstrap3">' + html_box + '</div>';
					$("#wr-add-element").remove();
					$('.jsn-modal').html(html_box);
					$('<div class="modal-footer ui-dialog-footerbar "><button type="button" class="btn btn-danger btn-close-modal" data-dismiss="modal">Close</button><button type="button" class="btn btn-primary btn-add-grid-custom">Add grid</button></div>').insertAfter($('.jsn-modal'));
					$('.ui-dialog-titlebar-close:last').html('×');
					$('.ui-dialog-titlebar-close:last').show();
					$('.ui-dialog-titlebar-close:last, .btn-close-modal').on('click', function(e) {
						e.preventDefault();
						grid_modal.close();
						$('.jsn-modal').remove();
						$('.custom-grid-active').removeClass('custom-grid-active');
						$.HandleElement.enablePageScroll();
					});
					$('.btn-add-grid-custom').on('click', function(e){
						e.preventDefault();
						if ( $('input#formGridColumns').val() == '' && $('input#formGridColumns').val() == null )
						{
							return false;
						}
						$('.wr-layout-thumbs .thumb-wrapper.custom-grid-active').attr('data-columns', $('input#formGridColumns').val());
						grid_modal.close();
						$('.jsn-modal').remove();
						$.HandleElement.enablePageScroll();
					...
					$(window).on('resize', function () {
						self.reCalculateSize('.wr-dialog', {width:400,height:225, contentHeight:80});
					});
				},
				fadeIn:200,
				scrollable: true,
		...
		addRow: function(self, this_wrapper){
			...
			this.wrapper.delegate('.wr-layout-thumbs .thumb-wrapper', 'click', function(event) {
				...
				if ( $(this).children().hasClass('wr-layout-gss-customs') )
				{
					if ( $(this).hasClass('custom-grid-active') )
					{
						$(this).removeClass('custom-grid-active');
					}
					else
					{
						$(this).addClass('custom-grid-active');
						self.addCustomGrid();
						return false;
					}						
		...
		updateSpanWidth:function(countColumn, totalWidth, parentForm){
			...
			parentForm.find(".jsn-column-container").each(function (i) {
				...
				if(i == parentForm.find(".jsn-column-container").length - 1)
					$(this).find('.jsn-column').css('width', Math.ceil(parseInt(selfSpan)*remainWidth/parseInt(gridColumns)) + 'px');
				else
					$(this).find('.jsn-column').css('width', Math.floor(parseInt(selfSpan)*remainWidth/parseInt(gridColumns)) + 'px');
			});
		},
		...
		_addColumn: function(column_html, target, position) {
			...
			if (countColumn < this.gridColumns) {
				countColumn++;

				var span = parseInt(this.gridColumns / countColumn);
				var exclude_span = (this.gridColumns % countColumn != 0) ? span + (this.gridColumns % countColumn) : span;

				// Update span old columns
				parentForm.find(".jsn-column-container").each(function() {
					$(this).attr('class', $(this).attr('class').replace(/span[0-9]{1,2,3,4,5,6}/g, 'span'+span));
					$(this).html($(this).html().replace(/span[0-9]{1,2,3,4,5,6}/g, 'span'+span));
				});

				// Update span new column
				column_html = $(column_html.replace(/span[0-9]{1,2,3,4,5,6}/g, 'span' + exclude_span));
		...
		_removeItem: function(this_wrapper, target, silent) {
			...
			if ($(target).hasClass('row')) {
				...
				var	span = parseInt(this.gridColumns / countColumn),
					exclude_span = (this.gridColumns % countColumn != 0) ? span + (this.gridColumns % countColumn) : span;
				...
				if (!$.HandleCommon.removeConfirmMsg($(target).parents(".jsn-column-container"), 'column', null, function() {
					...
					parentForm.find(".jsn-column-container").last().html(parentForm.find(".jsn-column-container").last().html().replace(/span[0-9]{1,2,3,4,5,6}/g, 'span'+exclude_span));
		...
		reCalculateSize: function (box, opt) {
			...
			if ( $('.jsn-modal:last').attr('id') == 'wr-add-element-modal' ) {
				...
				opt = $.extend({width:width, height: height}, opt);
				$(box).width(opt.width);
				$(box).height(opt.height);
				if ( typeof(opt.contentHeight) != 'undefined' )
				{
					$('#wr-add-element-modal').height(opt.contentHeight);
				}
				else
				{
				...
				$('.jsn-items-list', $(box)).height(opt.height - 160);
				$(box).css({
					top :'50%',
					left :'50%',
					margin :'-'+ (opt.height / 2) +'px 0 0 -'+ ( (opt.width / 2) - 7 ) +'px',
					'z-index': '100001'
				});
			}
		},
